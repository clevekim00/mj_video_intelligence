...
**한국어** | [English](architecture_en.md)
...
├── README.md
├── mj_video_intelligence/
│   ├── __init__.py        # 안정적인 공개 내보내기
│   ├── core.py            # 예산, 프로토콜, 도구, 에이전트 루프
│   └── openai.py          # OpenAI 호환 모델 어댑터
└── tests/
    └── test_core.py       # 호스트 애플리케이션 없이 실행되는 패키지 테스트
```
...
```text
호스트 애플리케이션
  ├─ 허용된 메타데이터·자막·프레임 자료 획득
  ├─ VideoTools 구현체 또는 IndexedVideoTools 생성
  ├─ VideoAgentModel 구현체 생성
  └─ 선택적으로 도메인 결과 검증기 주입
                  │
                  ▼
