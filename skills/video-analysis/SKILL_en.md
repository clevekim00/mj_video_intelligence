...
5. Read `result.json` and `report.md`. Explain status, evidence mode, coverage,
   ...
   timestamps when reporting findings. Read [result semantics](references/result_en.md)
...
Use Korean for user-facing reports by default and English when requested. Preserve original quotations and timestamps. Do not assume the CLI has a report-language option; provide a separate explanation from the JSON when needed.
